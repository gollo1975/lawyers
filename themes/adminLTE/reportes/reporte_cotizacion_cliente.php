...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Line(108,68,108,200);
        $this->Line(131,68,131,200);
        $this->Line(154,68,154,200);
    ...
    function EncabezadoDetalles() {
        $this->Ln(7);
        $header = array('CODIGO', 'REFERENCIA', 'CANTIDAD', 'VLR UNIT','SUBTOTAL', 'IVA', 'TOTAL');
        $this->SetFillColor(200, 200, 200);
        $this->SetTextColor(0);
    ...
    function Body($pdf,$model) {
        ...
        foreach ($detalles as $detalle) { 
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(15, 4, $detalle->codigo, 0, 0, 'L');
            $pdf->Cell(60, 4, utf8_decode($detalle->referencia) , 0, 0, 'L');
            $pdf->Cell(23, 4, $detalle->cantidad_referencia, 0, 0, 'R');
            $pdf->Cell(23, 4, number_format($detalle->valor_unidad, 0, '.', ','), 0, 0, 'R');
            $pdf->Cell(23, 4, number_format($detalle->subtotal, 0, '.', ','), 0, 0, 'R');
            $pdf->Cell(23, 4, number_format($detalle->impuesto, 0, '.', ','), 0, 0, 'R');
