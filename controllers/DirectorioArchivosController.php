...
class DirectorioArchivosController extends \yii\web\Controller
{
    ...
    public function actionIndex_imagen($codigo,$numero, $validador_imagen, $token)
    {
        ...
        $to = $count->count();
        return $this->render('index_imagen', [
            'model' => $model,
            'codigo' => $codigo,
            'numero' => $numero,
            'pagination' => $pages,
     ...
     public function actionSubir_imagen($token)
    {
        ...
        return $this->render("subir_imagen", ["model" => $model, "msg" => $msg, 'validador_imagen' => $validador_imagen, 'token' => $token]);
    }
    ...
    public function actionDescargar($id,$numero,$codigo, $token)
    {
            ...
            if (!$this->downloadFile($carpeta, $archivo->nombre, ["jpeg", "jpg", "png"]))
            {
        ...
        if(Yii::$app->request->post()){
            if((int) $idarchivodir)
            {
                ...
                if ($table) {
                    ...
                    $table->update();                       
                    $this->redirect(["directorio-archivos/index_imagen",'numero' => $numero,'codigo' => $codigo,'validador_imagen' => $validador_imagen, 'token' => $token]); 
